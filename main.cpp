...
#define lat1d 25.664092
#define lon1d 53.252084
...
{
    double lat1, lon1, lat2, lon2, delta_lon, delta_lat, central_ang, a, k;
    lat1 = degToRad(lat1d);
    lon1 = degToRad(lon1d);
    lat2 = degToRad(lat2d);
    lon2 = degToRad(lon2d);
    ...
    central_ang = 2 * atan2(sqrt(a), sqrt(1 - a));
    return (earth_radius * central_ang);
}

// structure which contains data and functions for accessing and processing data from the given customer file.
struct json
{
    ...
    void distance_calculator()
    {
        if (distanceEarth(lat2d, lon2d) <= 50.0000)
        {
            ...
            i++;
            out << "{\"user_id\": " << id[i - 1] << ", \"name\": " << name << ", \"Distance From You\": " << distanceEarth(lat2d, lon2d) << "}" << endl;
        }
    }
